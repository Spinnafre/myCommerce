...
export class UpdateProductUserCase implements IUpdateProductUseCase{
    ...
    async execute({id,name,price,qtd,category_id,oldCategories}:IProduct):Promise<void>{
        ...
        await this.productRepository.create({id,name,price,qtd,category_id,oldCategories})
    }
}