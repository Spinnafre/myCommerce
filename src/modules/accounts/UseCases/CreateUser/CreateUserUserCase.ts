...
export class CreateUserUserCase implements ICreateUserUserCase{
    ...
    async execute({name,username,address,email,password,isAdmin}:IUser):Promise<void>{
        const user = await this.userRepository.findByEmail(email)
        if(!!user){
            throw new AppErros('User already exists')
        }
        const passwordHash = await hash(password,8)
        await this.userRepository.create({ name, username, address, email, password:passwordHash, isAdmin })
